...
class qLowerBoundJointEntropySearch(AcquisitionFunction, MCSamplerMixin):
        ...
        **kwargs: Any,
        ...
        with warnings.catch_warnings():
            ...
            with fantasize_flag():
                ...
                # ..and this equates to the version by Tu et. al.
                else:
                    self.conditional_model = self.initial_model.condition_on_observations(
                        X=self.initial_model.transform_inputs(self.optimal_inputs),
                        Y=self.optimal_outputs,
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        if self.estimation_type == 'LB':
            ...
            raise ValueError(f'Estimation type {self.estimation_type} is not valid.'
    ...
    def _compute_lower_bound_information_gain(self, X: Tensor, return_parts: bool = False) -> Tensor:
        ...
        Returns:
            ...
            points `X`.
        """
        tkwargs = {"dtype": self.optimal_outputs.dtype,
        ...
        initial_entropy = 0.5 * torch.logdet(
            initial_posterior.mvn.covariance_matrix
        )

        # initial_entropy of shape batch_size or batch_size x num_models if FBGP
        # first need to unsqueeze the sample dim (1) and then the two last
        initial_entropy = initial_entropy.unsqueeze(1).unsqueeze(-1).unsqueeze(-1)
        ...
        var_truncated = noiseless_var * \
            (1 - normalized_mvs * pdf_mvs / cdf_mvs
             - torch.pow(pdf_mvs / cdf_mvs, 2)).clamp_min(CLAMP_LB)
        ...
        entropy_reduction = (
            initial_entropy - conditional_entropy.sum(dim=-2, keepdim=True)).squeeze(-1)
        # average over the number of optima and squeeze the q-batch
        
        entropy_reduction = entropy_reduction.mean(dim=1).squeeze(-1)
    ...
    def _compute_monte_carlo_variables(self, posterior):
        ...
        samples_log_prob = posterior.mvn.log_prob(
            samples.squeeze(-1)).unsqueeze(-1).unsqueeze(-1)
        return samples, samples_log_prob

    def _compute_monte_carlo_information_gain(self, X: Tensor, return_parts: bool = False) -> Tensor:
        r"""Evaluates the lower bound information gain at the design points `X`.
        ...
        initial_posterior = self.initial_model.posterior(
            X, observation_noise=True, posterior_transform=self.initial_model.outcome_transform)
        ...
        # initial_entropy of shape batch_size or batch_size x num_models if FBGP
        initial_entropy = initial_entropy.unsqueeze(
            -1).unsqueeze(MCMC_DIM).unsqueeze(-1)

        CLAMP_LB = torch.finfo(tkwargs["dtype"]).eps
        # Compute the mixture mean and variance
        posterior_m = self.conditional_model.posterior(
            X.unsqueeze(MCMC_DIM), observation_noise=True,
            posterior_transform=self.initial_model.outcome_transform)
        noiseless_var = self.conditional_model.posterior(
            X.unsqueeze(MCMC_DIM), observation_noise=False,
            posterior_transform=self.initial_model.outcome_transform
        ).variance
        ...
        mvs_rescaled_mc = (normalized_mvs - rho
                           * normalized_samples) / (1 - rho ** 2)
        cdf_mvs = normal.cdf(normalized_mvs).clamp_min(CLAMP_LB)
...
class qExploitLowerBoundJointEntropySearch(qLowerBoundJointEntropySearch):
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            estimation_type: A string to determine which entropy estimate is
                computed: Lower bound" or "Monte Carlo".
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        if self.exploit:
            res = self.initial_model.posterior(
                X.unsqueeze(MCMC_DIM)).mean.squeeze(-1).mean(-1)
        else:
            res = self._compute_lower_bound_information_gain(X)
